...
angular.module('archbrowserApp')
  .controller('MainCtrl', function ($scope, $http, flatDotViewer) {
    ...
    function initUi() {
      // set height of main view to window height, and adjust on window resize
      function setMainViewHeight() {
        $('.win').outerHeight($(window).innerHeight());
        visualize($scope.hits);
      }
      setMainViewHeight();
      $(window).bind('resize', setMainViewHeight);
      ...
      // make list handle mouse click selection
